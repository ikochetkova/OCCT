...
public:
  ...
  //! Set Supplemental geometry mode for indicate read Supplemental geometry or not.
  Standard_EXPORT void SetSupplementalMode(const Standard_Boolean supplementalMode);
...
protected:
  ...
  Standard_EXPORT Standard_Boolean
    ReadSupplemental(const Handle(XSControl_WorkSession)& WS,
                     const Handle(TDocStd_Document)&      doc,
                     const StepData_Factors&              theLocalFactors = StepData_Factors());
